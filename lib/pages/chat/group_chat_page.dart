...
class _ChatPageState extends State<ChatPage> {
  ...
  Future<void> pickImageFromGallery() async {
    ...
    if (pickedFile != null) {
      // Update the messageEditingController with the image file path
      setState(() {
        messageEditingController.text = pickedFile.path;
      });
    }
  }
  ...
  Future<void> _sendMessage() async {
    if (messageEditingController.text.isNotEmpty) {
      if (messageEditingController.text.startsWith("assets/") ||
          messageEditingController.text.startsWith("file:/")) {
        // Handle image message
        String imagePath = messageEditingController.text;
        Map<String, dynamic> chatImageMap = {
          "imagePath": imagePath,
          "sender": widget.userName,
          'time': DateTime.now().millisecondsSinceEpoch,
          'type': 1, // 1 represents an image message
        };

        await DatabaseService().sendMessage(widget.groupId, chatImageMap);
        ...
        Map<String, dynamic> chatMessageMap = {
          ...
          "sender": widget.userName,
          'time': DateTime.now().millisecondsSinceEpoch,
          'type': 0, // 0 represents a text message
        };
